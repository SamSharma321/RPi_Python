...
class VoiceEngine:
    ...
    def speak(self, audio, block = False):
        ...
        if audio != '':
            print(audio)
        myobj.save("/home/samram/jarvis2/src/test.mp3")
        if block:
            os.system("mpg321 -q /home/samram/jarvis2/src/test.mp3")
        else:
            subprocess.Popen(["mpg321", '-q', "/home/samram/jarvis2/src/test.mp3"])
        # os.remove("/home/samram/jarvis/test.mp3")
        #engine.say(audio)
        #engine.runAndWait()
    ...
    while True:
        ...
        if ('see me' in query) or ('find me' in query) or ('visible' in query):
            if (visibility is True) and ((datetime.now() - capture_time).total_seconds() < detection_timeout):
                ...
                jarvis.speak(spk[random.randint(0, 2)])
            else:
                spk = ['Searching', "Scanning room", "Looking for you now"]
                jarvis.speak(spk[random.randint(0, 2)])
                ret, frame = search()
                if ret is True:
                    jarvis.speak("Found you sir.")
                    capture_time = datetime.now()
                    detect_incr_cnt = 0
                else:
                    jarvis.speak("Not able to find you sir. Let me reset the camera for now")
                    detect_incr_cnt = detect_incr_cnt + 1
            ...
            my_info = open('/home/samram/jarvis/src/info.txt', 'r').read()
        ...
        elif ("remind me" in query) or (('add' in query) and ('reminder' in query)): # Use REGEX to get data bro
        ...
        elif r'check connection' in query:
        ...
        elif 'create a presentation' in query:# Doesn't work TODO
